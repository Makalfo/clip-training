...
def zero_shot_demo():
    ...
    parser.add_argument("--skimage", default=False, action='store_true', required=False, help="use skimage")
    ...
    if args.img_path:
        ...
        # use the skimage directory
        for filename in [filename for filename in os.listdir(skimage.data_dir) if filename.endswith(".png") or filename.endswith(".jpg")]:
            name = os.path.splitext(filename)[0]
            if name not in descriptions:
                continue

            image = transform(Image.open(os.path.join(skimage.data_dir, filename)).convert("RGB")).unsqueeze(0)

            # un normalized image for display
            raw_image = transform_no_norm(Image.open(os.path.join(skimage.data_dir, filename)).convert("RGB"))        
            raw_images.append(raw_image)
            images.append(image)
